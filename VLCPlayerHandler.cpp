...
void VLCPlayerHandler::loadMedia(const QString& mediaId) {
    ...
    // Base URL for the manifest with userID and token as query parameters
    QString baseUrl = QString("http://localhost:18080/media/%1/manifest?userID=%2&token=%3")
        .arg(mediaId)
        .arg(m_userId)
        .arg(m_token);

    // Convert the URL to a UTF-8 encoded byte array
